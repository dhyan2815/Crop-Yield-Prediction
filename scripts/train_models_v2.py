...
try:
    ...
    HAS_XGB = False
    print("WARNING: XGBoost not found. Falling back to Random Forest for v2 Champion model.")
...
def train_v2_models():
    ...
    if not os.path.exists(FEATURES_DATA_PATH):
        print(f"ERROR: Data not found at {FEATURES_DATA_PATH}. Please run feature engineering first.")
        return
    ...
    print(f"Training with {len(X.columns)} features, {len(X)} samples")
    ...
    print(f"Training Complete! R2={r2:.4f}, MAE={mae:.2f}")
